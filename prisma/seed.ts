...
async function main() {
  ...
  // Initial site configuration structure
  const defaultWeddingData = {
    ...
    weddingDate: '2026-12-18T16:00:00.000Z', // Dec 18, 2026 at 4:00 PM UTC (9:30 PM IST)
    events: [
      {
        id: '1',
    ...
    branding: {
      theme: 'maroon',
      primaryColor: '#800020', // Maroon
      secondaryColor: '#D4AF37', // Gold
      accentColor: '#FAF9F6', // Champagne/Ivory
      fontHeader: 'Cinzel Decorative', // Google Font for luxury header
      fontBody: 'Outfit', // Google Font for body readability
    },
    seo: {
  ...
  // Upsert website configurations
  const config = await prisma.siteConfig.upsert({
    where: { key: 'wedding_data' },
    update: { value: JSON.stringify(defaultWeddingData) },
    create: {
      key: 'wedding_data',
      value: JSON.stringify(defaultWeddingData),
    },
  });

  console.log('Seeded wedding configuration.');
}
